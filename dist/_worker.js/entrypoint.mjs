...
import { g as getActionQueryString, a as astroCalledServerError, A as ActionError, d as deserializeActionResult, b as ACTION_QUERY_PARAMS, c as appendForwardSlash } from './chunks/astro-designed-error-pages_Cfn6DnoG.mjs';
import { s as stringType, o as objectType, n as numberType } from './chunks/astro/server_CTXZP3YY.mjs';
import { a as appConfig } from './chunks/appConfig_ChyCgpio.mjs';
import { s as signPremiumExpiry, p as premiumSessionCookieName, v as verifyPremiumCookieValue, a as verifyPremiumFragmentSignature } from './chunks/premium-fragment-signing_DejlGN2E.mjs';
import { pbkdf2Sync, timingSafeEqual, createHash } from 'node:crypto';
import { d as defineAction } from './chunks/index_BoaZ9jzI.mjs';
...
`,e=e.substring(h.length+1),l=true),!l){let S=this.rules.other.nextBulletRegex(f),Y=this.rules.other.hrRegex(f),ee=this.rules.other.fencesBeginRegex(f),te=this.rules.other.headingBeginRegex(f),me=this.rules.other.htmlBeginRegex(f),xe=this.rules.other.blockquoteBeginRegex(f);for(;e;){let Z=e.split(`
...
const articleFileSlugSchema = stringType().trim().min(2).max(120).regex(/^[a-zA-Z0-9][a-zA-Z0-9._-]*$/);

function verifyPremiumPbkdf2Code(plain) {
  const pa = appConfig.premiumAccess;
  if (!pa?.enabled || !Array.isArray(pa.codes)) return false;
  const clean = String(plain || "").trim();
  if (!clean) return false;
  for (const entry of pa.codes) {
    if (entry.expiresAt) {
...
function verifyPremiumSessionCode(plain) {
  const clean = String(plain || "").trim();
  if (!clean) return false;
...
function clampScrollRatio(v) {
  if (v === null || v === void 0 || Number.isNaN(v)) return void 0;
  const x = Math.min(1, Math.max(0, v));
  return Math.round(x * 1e3) / 1e3;
}
async function reportPremiumArticleUnlocked(input) {
...
const premiumFragmentInputSchema = objectType({
  slug: articleFileSlugSchema,
  signature: stringType().trim().min(40).max(200),
  gaClientId: stringType().trim().max(120).optional(),
  scrollRatio: numberType().min(0).max(1).optional()
});
const startSessionInputSchema = objectType({
  code: stringType().trim().min(1).max(500)
});
const premium = {
  ...
  sessionStatus: defineAction({
    ...
    handler: async (_input, ctx) => {
      ...
      return { active: verifyPremiumCookieValue(raw) };
    }
  }),
  getFragment: defineAction({
    ...
    handler: async (input, ctx) => {
      ...
      const locks = premiumLocks;
      const md = locks[input.slug];
      if (md === void 0 || md.trim().length === 0) {
